...
public class BurningLayingSkeletonCorpseBlock extends HorizontalFacingBlock {
    public static final MapCodec<BurningLayingSkeletonCorpseBlock> CODEC = createCodec(BurningLayingSkeletonCorpseBlock::new);
    private static final VoxelShape SHAPE = Block.createCuboidShape(0.0, 0.0, 0.0, 16.0, 5.0, 16.0);

    public BurningLayingSkeletonCorpseBlock(Settings settings) {
        super(settings);
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        ...
        if (random.nextFloat() < 0.04F) { // 50% chance each tick
            ...
            world.addParticle(ParticleTypes.LAVA, x + offsetX, y, z + offsetZ, vx, vy, vz);
        }
    }
}
